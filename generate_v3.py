#!/usr/bin/env python3
"""Generate Bach chorales from v3 (pre-countdown) TonicNet weights.

Temporary script for evaluating the in-flight v3 training run.
Delete along with model_v3.py and weights-v3/ when no longer needed.

Usage:
    python generate_v3.py [n_samples] [--weights PATH] [--temperature T]
"""
...
from model_v3 import VOCABULARY, TonicNet
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="Generate from v3 (pre-countdown) TonicNet")
    parser.add_argument("n_samples", nargs="?", type=int, default=3,
                        help="Number of samples to generate (default: 3)")
    parser.add_argument("--weights", default="weights-v3/tonicnet-best.pt",
                        help="Path to v3 .pt weights")
    ...
    max_steps = args.bars * 80 + 80  # match v4 convention
    ...
    for i in range(args.n_samples):
        ...
        seq, reps, pos = model.generate(
            max_steps=max_steps, temperature=temperature, stop_on_end=True)
